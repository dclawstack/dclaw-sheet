...
### How do I update DClaw Sheet?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp sheet --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Sheet without Kubernetes?

Yes, for local development:

```bash
cd dclaw-sheet/frontend && npm run dev
cd dclaw-sheet/backend && uvicorn dclaw_sheet.main:app --reload
```
